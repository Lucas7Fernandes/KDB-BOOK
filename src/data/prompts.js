...
export function buildFluxPrompt(item, themeId, style = 'classic') {
  ...
  if (style === 'baby') {
    return [
      ...
      `- The ENTIRE drawing is made of clean BLACK OUTLINES on PURE WHITE (RGB 255,255,255) only`,
      ...
      `- Completely EMPTY pure white background: NO scenery, NO floor, NO ground line, NO textured or gray background rectangle, NO shadow under the character`,
      `- NO text, NO letters, NO words, NO watermark, NO signature, NO logo anywhere in the image`,
      `- Very simple, minimal interior detail, suitable for toddlers ages 1 to 5`,
      `- All shapes fully CLOSED so kids can color inside`,
      `COMPOSITION (very important for a consistent book): the baby ${item.en} is CENTERED and fills about 80 percent of a SQUARE frame,`,
      `full body always fully visible with a small even margin of white space on all four sides,`,
      `the character sized large and consistent so every page in the book matches, never small, never cropped or cut off at the edges.`,
      `Output: high-contrast crisp black-and-white coloring book line art on pure white, like a printable coloring page, square 1:1 composition.`,
    ].join(' ');
  }
